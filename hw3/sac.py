...
class SACAgent:
    ...
    def update_model(self) -> Tuple[torch.Tensor, ...]:
        ...
        with torch.no_grad():
            ...
            q_target = reward + self.gamma * mask * v_next
        ...
        with torch.no_grad():
            next_action, next_log_prob = self.actor(state)
            q1_pi = self.qf_1(state, next_action)
            q2_pi = self.qf_2(state, next_action)
            q_pi_min = torch.min(q1_pi, q2_pi)
            v_target = q_pi_min - alpha * next_log_prob
        ...
        if self.total_step % self.policy_update_freq == 0:
            ...
            q1_pi = self.qf_1(state, new_action)
            q2_pi = self.qf_2(state, new_action)
            q_pi_min = torch.min(q1_pi, q2_pi)
